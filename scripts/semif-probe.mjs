...
const embedded = [
  ...
  },
  {
    ...
    options: [
      { id: "low", description: "Not time sensitive." },
      { id: "high", description: "Explicitly time sensitive." },
    ],
  },
  {
    id: "quality-1",
    state: "A bug report with a reproduction and a stack trace.",
    question: "How actionable is this report?",
    options: [
      { id: "unusable", description: "No repro." },
      { id: "partial", description: "Some detail." },
      { id: "actionable", description: "Reproduction and trace." },
    ],
  },
];
...
for (const row of rows) {
  if (!row.id || !row.state || !row.question || !Array.isArray(row.options)) {
    failures.push(`${row.id ?? "?"}: row is not in the SemIf schema`);
    continue;
  }
  let response;
  let payload;
  ...
  if (!response.ok) {
    failures.push(`${row.id}: HTTP ${response.status} ${JSON.stringify(payload).slice(0, 200)}`);
    continue;
  }
  ...
  if (probabilities.length !== expected.length) {
    failures.push(`${row.id}: ${probabilities.length} probabilities for ${expected.length} options`);
    continue;
  }
