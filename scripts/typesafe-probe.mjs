...
const rows = [
  ...
  {
    id: "score-1",
    state: "A crisp bug report with a reproduction and a stack trace.",
    questions: {
      quality: { type: "score", instructions: "How actionable is this report?", criteria: ["Unusable", "Partial", "Actionable"] },
    },
  },
];
...
for (const row of rows) {
  ...
  for (const [id, question] of Object.entries(row.questions)) {
    ...
    if (!answer) {
      failures.push(`${row.id}: no answer for "${id}"`);
      continue;
    }
    ...
    if (question.type === "noul") {
      if (typeof answer.noul !== "number" || answer.noul < 0 || answer.noul > 1)
        failures.push(`${row.id}.${id}: noul ${answer.noul} outside 0..1`);
      continue;
    }
    ...
    if (ids.length !== expected.length)
      failures.push(`${row.id}.${id}: ${ids.length} probabilities for ${expected.length} options (${ids.join(",")})`);
