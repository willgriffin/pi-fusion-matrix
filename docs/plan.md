...
- **it reads what omp recorded, rather than what omp's record looks like.** omp invokes an extension tool
  ...
  call whose result never reached the store still counts and the line agrees with the tools table;
...
could not attribute), `2` `--check` failed. `--check` is the reader's own accounting, checked against fixtures (80 checks as a normal
user; 74 as root, where the six permission-dependent cases are printed as skipped rather than counted — the
printed `N/N` is always the real total, and 74 is arithmetic from the guarded count, not a measured root run) — and, as
...
22. **The run record survives, and the report reads it** — `node scripts/session-report.mjs --check`
    passes 80/80 (74/74 as root by arithmetic — six permission cases are skipped), each check having been shown
