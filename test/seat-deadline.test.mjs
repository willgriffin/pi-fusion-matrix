...
const fakeModel = (provider, id) => ({
  id, name: id, api: "openai-completions", provider, baseUrl: "http://127.0.0.1:1/v1",
 ...
 */
const silentProvider = (args) => new Promise((_, reject) => {
  ...
  args.signal.addEventListener("abort", () => { clearTimeout(giveUp); reject(new Error("the request was aborted")); }, { once: true });
});

const seat = (over = {}) => runSeat({
  personaName: "skeptic", persona: { name: "skeptic", prompt: "p", temperature: 0.7 },
  candidates: ["deepseek-pro"], fusion: { id: "f", seatTimeoutMs: 40 }, config, registry,
  callModel: silentProvider, decide: async () => ({ answers: {}, usage: {} }), emit,
  vars: { prompt: "a packet" }, maxAdvance: 3, ...over,
});
...
test("a seat that never answers fails with reason timeout instead of hanging", async () => {
  ...
  assert.deepEqual(result.attempts.map((a) => a.reason), ["timeout"]);
...
test("a hanging candidate advances to the next one, and each attempt is recorded", async () => {
  ...
  assert.deepEqual(result.attempts.map((a) => a.reason), ["timeout", "timeout", "timeout"]);
  ...
  assert.deepEqual(result.substitutions.map((s) => [s.from, s.to]),
    [["glm@opencode-go", "glm@zai"], ["glm@zai", "deepseek-pro@opencode-go"], ["deepseek-pro@opencode-go", "—"]]);
});
