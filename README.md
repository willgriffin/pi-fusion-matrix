...
reported either way, and the exit status is non-zero when something could not be accounted for, so a
filtered report cannot pass a short total off as a fact about the fusions.
